...
//生成起始点列表 在两个头上各绕一圈
int CMfcogl1Doc::OnGenerateTubeStartList(std::deque<struct tubePathPosition>& TubeStartList) {
...
void CMfcogl1Doc::OnComputeFiberPathTube() {
	...
	if (m_bCanComputing == true)
	{
		...
		*/
		TubePointList = new std::deque<std::deque<struct tubePathCoord>*>;//TubePoint的双线链表指针的双向链表，内层表示一条纤维路径
		TubeTrackList = new std::deque<std::deque<struct track>*>;//TubePoint的双线链表指针的双向链表，内层表示一条纤维路径
		TubeTrackListTime = new std::deque<struct track>;
		TubePointListTime = new std::deque<struct tubePathCoord>;
		//debug
...
//根据当前位置和跳跃数算出本条路径的起点
void CMfcogl1Doc::OnGeneratePosition(std::deque<struct tubePathPosition>&TubeStartList) {

	float3 trackStart = { cos(-currentAngle), sin(-currentAngle),0 };
...
int CMfcogl1Doc::OnRenderLinePart(int state, std::deque<struct tubePathCoord>* singlePathList) {
	...
	case 1:
		...
		while (nextPoint[0] < model.a && 0 <= nextPoint[2] && nextPoint[2] <= model.length) {
			...
			nextPoint[0] = startPoint[0] + model.curveLength * sin(model.angle);
			nextPoint[1] = startPoint[1];
			nextPoint[2] = startPoint[2] + model.curveLength * cos(model.angle) * direction;
			...
			singlePathList->push_back(tempTubePoint);

			model.curveLength += windingPathStep;
		}
	...
	case 3:
		...
		while (-model.b < nextPoint[1] && 0 <= nextPoint[2] && nextPoint[2] <= model.length) {
			...
			nextPoint[1] = startPoint[1] - model.curveLength * sin(model.angle);
			nextPoint[2] = startPoint[2] + model.curveLength * cos(model.angle) * direction;
...
int CMfcogl1Doc::OnRenderCurvePart(int state, std::deque<struct tubePathCoord>* singlePathList) {
	...
	case 2:
		...
		model.curveLength = phase;
		startPoint[0] = model.a;
		startPoint[1] = model.b;

		//圆柱面上测地线 对弧面的三条边约束
		while (model.curveLength <= PI / 2 && 0 <= nextPoint[2] && nextPoint[2] <= model.length) {
			insertPoint(nextPoint);

			//0:横向（2a）1:高（2b）2:纵向（C）angle:缠绕角 nextPoint:递推 m_view_r:倒角半径
			nextPoint[0] = startPoint[0] + model.r * sin(model.curveLength);
			nextPoint[1] = startPoint[1] + model.r * cos(model.curveLength);
			nextPoint[2] = startPoint[2] + model.r * (model.curveLength - phase) / tan(model.angle) * direction;

			tempTubePoint.x = nextPoint[0];
			tempTubePoint.y = nextPoint[1];
			tempTubePoint.z = nextPoint[2];
			tempTubePoint.normal_radian = -model.curveLength + PI / 2;
			tempTubePoint.tangent.x = sin(model.angle) * cos(tempTubePoint.normal_radian - PI / 2);
			tempTubePoint.tangent.y = sin(model.angle) * sin(tempTubePoint.normal_radian - PI / 2);
			...
			model.curveLength += windingPathStep/adjust;
		}
	...
	case 4:
		...
		//圆柱面上测地线
		while (model.curveLength <= PI && 0 <= nextPoint[2] && nextPoint[2] <= model.length) {
			insertPoint(nextPoint);
	...
	case 6:
		...
		//圆柱面上测地线
		while (model.curveLength <= 1.5 * PI && 0 <= nextPoint[2] && nextPoint[2] <= model.length) {
			insertPoint(nextPoint);
	...
	case 8:
		...
		//圆柱面上测地线
		while (model.curveLength <= 2 * PI && 0 <= nextPoint[2] && nextPoint[2] <= model.length) {
			//0:横向（2a）1:高（2b）2:纵向（c）angle:缠绕角 nextPoint:递推 m_view_r:倒角半径
			insertPoint(nextPoint);
